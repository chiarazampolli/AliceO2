...
struct HFCandidateCreatorCascade {
  ...
  )
  {
    ...
    for (const auto& casc : rowsTrackIndexCasc) {
      ...
      auto indexV0DaughPos = posTrack.mcParticleId();
      auto indexV0DaughNeg = negTrack.mcParticleId();
      bool isLc = isLcK0SpFunc(indexBach, indexV0DaughPos, indexV0DaughNeg, indexProton, indexK0Spos, indexK0Sneg);
#endif

      MY_DEBUG_MSG(isLc, LOG(INFO) << "Processing the Lc with proton " << indexBach << " posTrack " << indexV0DaughPos << " negTrack " << indexV0DaughNeg);

      auto trackParCovBach = getTrackParCov(bach);
      auto trackParCovV0DaughPos = getTrackParCov(posTrack); // check that MyBigTracks does not need TracksExtended!
      auto trackParCovV0DaughNeg = getTrackParCov(negTrack); // check that MyBigTracks does not need TracksExtended!
      ...
      if (df.process(trackV0, trackParCovBach) == 0) {
        ...
        //	if (isLc) {
        // LOG(INFO) << "Vertexing failed for Lc with proton " << indexBach << " posTrack " << indexV0DaughPos << " negTrack " << indexV0DaughNeg;
        //}
        continue;
      } else {
      ...
      MY_DEBUG_MSG(isLc, LOG(INFO) << "IT IS A Lc! Filling for Lc with proton " << indexBach << " posTrack " << indexV0DaughPos << " negTrack " << indexV0DaughNeg);
...
struct HFCandidateCreatorCascadeMC {
               ...
               aod::McParticles const& particlesMC)
  {
    ...
    for (auto& candidate : candidates) {
      ...
      auto indexV0DaughPos = candidate.posTrack_as<aod::BigTracksMC>().mcParticleId();
      auto indexV0DaughNeg = candidate.negTrack_as<aod::BigTracksMC>().mcParticleId();
      auto indexBach = candidate.index0_as<aod::BigTracksMC>().mcParticleId();
      ...
      if (sign != 0) { // we have already positively checked the K0s
        ...
        auto indexProton = candidate.index0_as<aod::BigTracksMC>().mcParticleId();
        MY_DEBUG_MSG(sign, LOG(INFO) << "index proton = " << indexProton);
